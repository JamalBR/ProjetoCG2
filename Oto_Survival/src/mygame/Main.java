...
public class Main
    ...
    private boolean up = false, down = false, left = false, right = false, reinicia = false, pausar = false, isRunning = true, fim = false;    
    private Material boxMatColosion;
    AudioNode somTiro;    
    public ArrayList<Integer> recordesPontos = new ArrayList<Integer>();
    private int volume = 5, placar = 0;
    ...
    public void simpleUpdate(float tpf) {
        ...
        if (!pausar && !fim) {
            ...
            if (vida == 0) {
                ...
                //pausar = true;
                addRecords();
                System.out.println(recordesPontos.size());
                fim = true;
            }
        }else if(fim)
        {
    ...
    public void menu() {
        ...
        if (value == 3) {        
            ...
            optionList.add("ok");           
            Object[] rc = optionsList.toArray();
            //Collections.sort(recordesPontos);
            
            for(int i = 0; i < recordesPontos.size(); i++)
            {
                rankingList += recordesPontos.get(i).toString() + "\n";
            }
        ...
        if (value == 4) {
            JOptionPane.showMessageDialog(null,
                    ...
                    + "P - Pausar\n"
                    + "Objetivo: "
                    + "Não deixar nenhum bloco cair no chão, senão o jogo uma vida será perdida.\n"
                    + "Caso todas sua vida chegue a 0 o jogo acaba.\n"
